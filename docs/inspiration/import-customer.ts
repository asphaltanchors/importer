...
import { AddressData, BaseImportStats, ImportContext } from './shared/types.js';
import { BatchEmailProcessor, BatchPhoneProcessor, DEFAULT_BATCH_SIZE } from './shared/batch-utils.js';
import { BatchAddressProcessor } from './shared/address-processor.js';
import { 
  createCsvParser, 
  parseDate, 
  processAddress, 
...
interface CustomerImportStats extends BaseImportStats {
  ...
  processed: number;  // From BaseImportStats
  warnings: string[]; // From BaseImportStats
}
...
interface CustomerImportContext extends ImportContext {
  ...
  prisma: PrismaClient; // From ImportContext
  debug: boolean;      // From ImportContext
  stats: CustomerImportStats; // Override with more specific type
}
...
async function importCustomers(filePath: string, debug: boolean, options: { skipLines?: number }): Promise<void> {
  ...
  await processImport<CustomerRow>(ctx, parser, async (row: CustomerRow) => {
    ...
    ctx.pendingContactInfo.set(quickbooksId, {
      emails: [
        ...mainEmails.map((email: string, index: number) => ({
          email,
          type: EmailType.MAIN,
          isPrimary: index === 0
        })),
